...
transaction(nameHash: String, bio: String) {
    ...
    prepare(account: AuthAccount) {
        ...
        let collectionPvt = account.borrow<&{Domains.CollectionPrivate}>(from: Domains.DomainsStoragePath) ?? panic("Could not load private collection")
...
transaction(name: String, duration: UFix64) {
    ...
    prepare(account: AuthAccount) {
        ...
        self.domain = domain!
        let vaultRef = account.borrow<&FungibleToken.Vault>(from: /storage/flowTokenVault) ?? panic("Could not borrow flow token vault")
        let rentCost = Domains.getRentCost(name: name, duration: duration)
        self.vault <- vaultRef.withdraw(amount: rentCost)
    }
