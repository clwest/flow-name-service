...
export default function Home() {
    // Use the AuthContext to track user data a
    const {currentUser, isInitialized, checkInit} = useAuth();
    const [domainInfos, setDomainInfos] = useState([])

    // Function to initialize teh user's account if not already isInitialized
                ...
                {!isInitialized ? (
                    ...
                    <div className={styles.domainsContainer}>
                        {domainInfos.length === 0 ? (
                            <p>You have not registered any FNS Domain yet</p>
                        ) : (
