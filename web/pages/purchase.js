...
export default function Purchase() {
    // use the Auth Context to check whether the connected user is initialized or not
    const { isInitialized, checkInit } = useAuth();
    // State variables to keep track of the domain name the user wants
    const [name, setName] = useState("");
    // State variable to keep track of how many years
    // teh user wants to rent the domain for
    const [years, setYears] = useState(1);
    // State variable to keep track of teh cost of the purachse
    ...
    async function initialize() {
        try {
            ...
            // This method waits for the transactions to be mined (sealed)
            await fcl.tx(txId).onceSealed();
            // Recheck account initialization afte transtions goes through
            await checkInit()
        } catch (error) {
            console.error(error)
        }
    ...
    // Call getCost() every time name and years change 
    useEffect(() => {
        getCost();
    }, [name, years])
